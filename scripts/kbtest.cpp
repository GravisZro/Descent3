...
char *Object_names[NUM_OBJECT_NAMES] = {"SeeBox"};
...
char *Message_names[NUM_MESSAGE_NAMES] = {"CanSee", "CantSee"};
...
short LevelScript_0000::CallEvent(int event, tOSIRISEventInfo *data) {
  ...
  case EVT_INTERVAL: {
    ...
    // Script 000: Can See Obj
    if (1) {
      if (qObjCanSeeObj(qGetPlayerObj(0), Object_handles[0], 90) == true) {
        aShowHUDMessage(Message_strings[0]);
      } else {
        aShowHUDMessage(Message_strings[1]);
      }
      aTogglePlayerControl(0, 0, 16896);
