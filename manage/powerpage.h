...
struct mngs_power_page {
  ...
  pagename_t sound_name[MAX_POWERUP_SOUNDS];
};
...
int mng_ReplacePowPage(const pagename_t& name, int handle, int local);

// Given a powerup name, finds it in the table file and deletes it
// If local is 1, deletes from the local table file
int mng_DeletePowPage(const pagename_t& name, int local);

// Reads in the powpage named "name" into powpage struct
// Returns 0 on error or couldn't find, else 1 if all is good
int mng_FindSpecificPowPage(const pagename_t& name, mngs_power_page *powpage, int offset = 0);
...
int mng_GetGuaranteedPowerupPage(const pagename_t& name, CFILE *infile = NULL);
