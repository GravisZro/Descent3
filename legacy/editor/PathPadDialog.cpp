...
void CPathPadDialog::OnPathpadAddPath()
{
        ...
        if(dup_test != -1)
        {
          OutrageMessageBox("PATH NOT CREATED: A path named %s already exists.", std::data(namestr));
                FreeGamePath(pathnum);
                return;
        }
        GamePaths[pathnum].name = namestr;
...
void CPathPadDialog::OnSelendokPathpadPulldown()
{
  ...
  SendDlgItemMessage( IDC_PATHPAD_PULLDOWN, CB_GETLBTEXT,cur,(LPARAM) (LPCTSTR)std::data(name));
