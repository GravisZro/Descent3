...
void CWorldObjectsGenericDialog::UpdateDialog() 
{
	...
	if(Object_info[m_current].dspew[0] >= 0 && Object_info[m_current].dspew[0] < MAX_OBJECT_IDS && Object_info[Object_info[m_current].dspew[0]].type != OBJ_NONE)
		SendDlgItemMessage( IDC_DEATH_POWERUP1_PULLDOWN, CB_SELECTSTRING,0,(LPARAM) (LPCTSTR) Object_info[Object_info[m_current].dspew[0]].name);
	else
	...
	if(Object_info[m_current].dspew[1] >= 0 && Object_info[m_current].dspew[1] < MAX_OBJECT_IDS && Object_info[Object_info[m_current].dspew[1]].type != OBJ_NONE)
		SendDlgItemMessage( IDC_DEATH_POWERUP2_PULLDOWN, CB_SELECTSTRING,0,(LPARAM) (LPCTSTR) Object_info[Object_info[m_current].dspew[1]].name);
	else
	...
	//Weapon Info box
  	((CButton *)GetDlgItem(IDC_GENERIC_WEAPON_INFO_BUTTON))->EnableWindow(Poly_models[Object_info[m_current].render_handle].num_wbs > 0);
...
//	((CEdit *) GetDlgItem(IDC_GENERIC_SCRIPT_ID_EDIT))->SetWindowText(txt);
	...
	if (first != -1) {
		...
		n = SendDlgItemMessage( IDC_GENERIC_NAME_PULLDOWN, CB_FINDSTRINGEXACT,0,(LPARAM) (LPCTSTR) Object_info[m_current].name);
		ASSERT(n != CB_ERR);
		...
		else
		{
			...
			if (mng_FindSpecificGenericPage (temp_pl.name,&page))
			{
				if (mng_AssignGenericPageToObjInfo (&page,m_current))
				{
					...
					OutrageMessageBox ("There was a problem loading this object.  You might encounter problems in dealing with it.  Good luck!");
...
void CWorldObjectsGenericDialog::OnGenericChangeModel() 
{
	...
	if ((FindPolyModelName(curname))!=-1)
	{
		...
		int answer=MessageBox ("There is already a model in memory with this name.  Are you sure you want to replace it?",fname,MB_YESNO);
		if (answer==IDNO)
			return;
	}
...
void CWorldObjectsGenericDialog::OnGenericPaste() 
{
	...
	if (Copy_object.type != m_type) {
		if (OutrageMessageBox (MBOX_YESNO, "You are about to paste a %s object as a %s.  Is this OK?",Object_type_names[Copy_object.type],Object_type_names[m_type]) != IDYES)
			return;
	}
...
BOOL CWorldObjectsGenericDialog::DestroyWindow() 
{
...
{
	RobotEditWeaponsDialog dlg(Object_info[m_current].static_wb, GetPolymodelPointer (Object_info[m_current].render_handle));
