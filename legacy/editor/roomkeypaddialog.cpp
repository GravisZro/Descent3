...
CRoomKeypadDialog::CRoomKeypadDialog(CWnd* pParent /*=NULL*/)
	: CKeypadDialog(CRoomKeypadDialog::IDD, pParent)
{
...
void CRoomKeypadDialog::OnLoadRoom() 
{
	...
	if (val!=-1)
	{
		int answer=MessageBox ("Do you wish to overwrite the room in memory that already has this name?",roomname,MB_YESNO);
		if (answer==IDYES)
			FreeRoom(&Rooms[val]);
		else 
			find_new=1;
...
void CRoomKeypadDialog::OnSize(UINT nType, int cx, int cy) 
{
...
void CRoomKeypadDialog::OnMarkRoom() 
{
	SetMarkedRoom();
}

void CRoomKeypadDialog::OnRoompadRotateLeft() 
{
	RotateRooms(0,(angle)-512,0);
}

void CRoomKeypadDialog::OnRoompadRotateRight() 
{
	RotateRooms(0,(angle)512,0);
}

void CRoomKeypadDialog::OnRoompadPitchForward() 
{
	RotateRooms((angle)512,0,0);
}

void CRoomKeypadDialog::OnRoompadPitchBackward() 
{
	RotateRooms((angle)-512,0,0);
}

void CRoomKeypadDialog::OnRoompadBankRight() 
{
	RotateRooms(0,0,(angle)512);
}

void CRoomKeypadDialog::OnRoompadBankLeft() 
{
	RotateRooms(0,0,(angle)-512);
}
...
BOOL CRoomKeypadDialog::OnInitDialog() 
{
	...
	i=box->AddString("None");						box->SetItemData(i,PD_NONE);
	i=box->AddString("Energy Weapon");			box->SetItemData(i,PD_ENERGY_WEAPON);
	i=box->AddString("Matter Weapon");			box->SetItemData(i,PD_MATTER_WEAPON);
	i=box->AddString("Melee Attach");			box->SetItemData(i,PD_MELEE_ATTACK);	
	i=box->AddString("Concussive Force");		box->SetItemData(i,PD_CONCUSSIVE_FORCE);
	i=box->AddString("Wall Hit");					box->SetItemData(i,PD_WALL_HIT);
...
void CRoomKeypadDialog::OnGoalfaceCheck() 
{
	...
	if (btn->GetCheck())
	{
		if (!(Curroomp->flags & GOALROOM|RF_SPECIAL1|RF_SPECIAL2|RF_SPECIAL3|RF_SPECIAL4|RF_SPECIAL5|RF_SPECIAL6))
		{
