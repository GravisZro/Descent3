...
CScriptSyncDialog::CScriptSyncDialog(CWnd* pParent /*=NULL*/)
	: CDialog(CScriptSyncDialog::IDD, pParent)
{
...
bool ManageFindNext(char *buffer)
{
	...
	{
		if(GlobalTrackLocks[ManageFindInData.curr_index].used && GlobalTrackLocks[ManageFindInData.curr_index].pagetype==PAGETYPE_GAMEFILE && 
...
void ManageFindClose(void)
{
	ManageFindInData.searching = false;
}
...
void SS_CompileOutputCallback(char *str)
{
	SS_CompileOutputData += str;
}
			...
			case STATE_OOS:
				{
					...
					if(m_HasCompiler)
					{
						...
						if(m_Files[m_Index].state == STATE_MISSING||m_Files[m_Index].state == STATE_ERROOD||m_Files[m_Index].state == STATE_ERROOS)
						{
							m_ErrorCount++;
							char *string;					
