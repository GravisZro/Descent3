...
CViewerPropDlg::CViewerPropDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CViewerPropDlg::IDD, pParent)
{
...
void CViewerPropDlg::OnAlignXnegButton() 
{
	...
	m->fvec.x = -1.0; m->fvec.y = m->fvec.z = 0;
	m->uvec.y =  1.0; m->uvec.x = m->uvec.z = 0;
	m->rvec.z =  1.0; m->rvec.x = m->rvec.y = 0;
...
void CViewerPropDlg::OnAlignYnegButton() 
{
	...
	m->fvec.y = -1.0; m->fvec.x = m->fvec.z = 0;
	m->uvec.z =  1.0; m->uvec.x = m->uvec.y = 0;
	m->rvec.x =  1.0; m->rvec.y = m->rvec.z = 0;
...
void CViewerPropDlg::OnAlignZnegButton() 
{
	...
	m->fvec.z = -1.0; m->fvec.x = m->fvec.y = 0;
	m->uvec.y =  1.0; m->uvec.x = m->uvec.z = 0;
	m->rvec.x = -1.0; m->rvec.y = m->rvec.z = 0;
