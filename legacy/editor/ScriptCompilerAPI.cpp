...
void outputtofile(CFILE *CurrentFile, char *format, ...)
{
	char buffer[1024];
	va_list marker;
...
bool ScriptCreateEmptyLevelScript(char *filename)
{
	...
	ddio_MakePath(fullpath,LocalScriptDir,filename,NULL);
        CFILE *CurrentFile = cfopen(fullpath,"wt");
	if(!CurrentFile)
		return false;

        outputtofile(CurrentFile, "// %s", filename);
        outputtofile(CurrentFile, "// %s", VERSION);
        cf_WriteString(CurrentFile,
R"code(#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int     STDCALL GetGOScriptID(const pagename_t& name,uint8_t is_door);
...
char STDCALL InitializeDLL(tOSIRISModuleInit *func_list)
{
  osicommon_Initialize(func_list);
...
// The only reserved ID is 0, which must be used for the level script
...
// CreateInstance
// Purpose:
//  Given an ID from a call to GetGOScriptID(), GetTriggerScriptID() or GetCOScriptList(), this
// function will create a new instance for that particular script (by allocating and initializing
// memory, etc.).  A pointer to this instance is to be returned back to Descent 3.  This pointer will
// be passed around, along with the ID for CallInstanceEvent() and DestroyInstance().  Return NULL
...
// GetTriggerScriptID
// Purpose:
//  Given a room and face number, this function will return a unique ID (global DLL unique)
// in which Descent 3 should use for all future interaction will the DLL when refering to
// this trigger.  Return -1 if there is no trigger script available for the specified trigger.
// The only reserved ID is 0, which must be used for the level script
int STDCALL GetTriggerScriptID(int trigger_room,int trigger_face)
{
  return -1;
...
// GetCOScriptList
// Purpose:
//  This function returns the pointers to the 2 arrays that Descent 3 should use to determine
// what object's have custom scripts in this level DLL.  list should be set to point to the array
// of object handles that have custom scripts in this DLL.  id_list should be set to point to the
// corresponding array of unique IDs that match with the object handle list. This function should
// return the size of the arrays returned...if no custom scripts are available, than this function
// returns 0.
// The only reserved ID is 0, which must be used for the level script
int STDCALL GetCOScriptList( int **list, int **id_list )
{
