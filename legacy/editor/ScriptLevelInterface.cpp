...
CScriptLevelInterface::CScriptLevelInterface(CWnd* pParent /*=NULL*/)
	: CDialog(CScriptLevelInterface::IDD, pParent)
{
...
void CScriptLevelInterface::OnRadioScripts() 
{
	UpdateDialog();
}

void CScriptLevelInterface::OnRadioLevels() 
{
	UpdateDialog();
}
			...
			if(MessageBox(buffer,"Congratulations",MB_YESNO)==IDYES){
				...
				if(!cfexist(buffer)){
					...
					if(AddNewGameFile(buffer,"scripts")){
						sprintf(buffer,"%s has been added into the manage system.  Don't forget to check it in to make sure it stays in the\nmanage system!",filename);
						MessageBox(buffer,"Success",MB_OK);
					}else{
						sprintf(buffer,"There was a problem adding %s\ninto the manage system. You'll have\nto do it automatically.",filename);
						MessageBox(buffer,"Error",MB_OK);
					}
				}
			}
		}else{
			sprintf(buffer,"Doh! Congratulations! You compiled the DLL successfully.  However,\nI don't see it in the manage system.  I'd offer to add it\nautomatically for you, but you said NO to the\ndata update.  You'll have to add it manually.");
...
void CScriptLevelInterface::OnSelchangeSelectedScript() 
{	
	OnCloseupSelectedScript();	
}
...
{	
	...
	for (int i=0;i<MAX_GAMEFILES;i++){
		if (Gamefiles[i].used){
			...
			if(!stricmp(&Gamefiles[i].name[length-4],".d3l") && (mng_FindTrackLock(Gamefiles[i].name,PAGETYPE_GAMEFILE)==-1) ){
...
void CScriptLevelInterface::UpdateAvailableWithScripts()
{
	...
	for (int i=0;i<MAX_GAMEFILES;i++){
		if (Gamefiles[i].used){
			//we're only adding .d3l's to this list
			length = strlen(Gamefiles[i].name);
			if(!stricmp(&Gamefiles[i].name[length-4],".cpp") && (mng_FindTrackLock(Gamefiles[i].name,PAGETYPE_GAMEFILE)==-1) ){
				strcpy(buffer,Gamefiles[i].name);
				buffer[length-4] = '\0';
				list->AddString(buffer);
			}
...
void CScriptLevelInterface::OnCreatescript() 
{
	...
	if(dlg.DoModal()){
		...
		if(i<MAX_GAMEFILES || (mng_FindTrackLock(filename,PAGETYPE_GAMEFILE)!=-1) ){
			char buffer[_MAX_PATH];
			sprintf(buffer,"Just so you know, %s\nalready exists in the manage system, do you still want\nto create a new file of the same name?",filename);
			on_network = true;
...
void CScriptLevelInterface::OnDestroy() 
{
	CDialog::OnDestroy();	
}
...
void CScriptLevelInterface::OnListNoncheckedOut() 
{
	UpdateDialog();
}
	...
	else 
	{
		...
		if ( mng_FindSpecificGamefilePage (temp_pl.name,&gamefilepage,0))
		{
			if (mng_AssignGamefilePageToGamefile (&gamefilepage,n))
			{
				...
				OutrageMessageBox ("There was a problem loading this gamefile(%s).  You might encounter problems in dealing with it.  Good luck!",tempbuffer);
...
bool UndoCheckOutGamefile(char *tempbuffer)
{
	...
	if (OutrageMessageBox(MBOX_YESNO,"Are you sure you want to undo your lock on %s and lose any changes you may have made?",tempbuffer) != IDYES)
		return false;
...
bool CScriptLevelInterface::DeleteLevel()
{
	...
	for (i = 0; i < list_size; i++ ){
		...
		for(j=0;j<MAX_TRACKLOCKS;j++){
			if( (GlobalTrackLocks[j].used) && (GlobalTrackLocks[j].pagetype==PAGETYPE_GAMEFILE) && (!stricmp(GlobalTrackLocks[j].name,tempbuffer)) ){
	...
	for (i = 0; i < list_size; i++ ){
		...
		for(j=0;j<MAX_TRACKLOCKS;j++){
			if( (GlobalTrackLocks[j].used) && (GlobalTrackLocks[j].pagetype==PAGETYPE_GAMEFILE) && (!stricmp(GlobalTrackLocks[j].name,tempbuffer)) ){
				//ok, we found a possibly matching script...ask if they want that
				//checked in also...
				sprintf(buffer,"A possible level script (%s) was found, do you want\nto check this in also? (HIGHLY recommended)",tempbuffer);
				if(m_AutoCheckScripts || MessageBox(buffer,"Friendly Warning",MB_YESNO)==IDYES){
					char t[256];
					sprintf(t,"Checking In: %s",tempbuffer);
					SetStatus(t);
...
{	
	...
	for (i = 0; i < list_size; i++ ){
		...
		for(j=0;j<MAX_GAMEFILES;j++){
			if( (Gamefiles[j].used) && (!stricmp(Gamefiles[j].name,tempbuffer)) && (mng_FindTrackLock(tempbuffer,PAGETYPE_GAMEFILE)==-1)){
				//ok, we found a possibly matching script...ask if they want that
				//checked out also...
				//go ahead and delete the sucker!
				char t[256];
	...
	for (i = 0; i < list_size; i++ ){
		...
		for(j=0;j<MAX_GAMEFILES;j++){
			if( (Gamefiles[j].used) && (!stricmp(Gamefiles[j].name,tempbuffer)) && (mng_FindTrackLock(tempbuffer,PAGETYPE_GAMEFILE)==-1)){
				//ok, we found a possibly matching script...ask if they want that
				//checked out also...
				sprintf(buffer,"A possible level script (%s) was found, do you want\nto check this out also? (HIGHLY recommended)",tempbuffer);
				if(m_AutoCheckScripts || MessageBox(buffer,"Friendly Warning",MB_YESNO)==IDYES){
					//go ahead and delete the sucker!
					char t[256];
					sprintf(t,"Checking Out: %s",tempbuffer);
	...
	for (i = 0; i < list_size; i++ ){
		...
		for(j=0;j<MAX_TRACKLOCKS;j++){
			if( (GlobalTrackLocks[j].used) && (GlobalTrackLocks[j].pagetype==PAGETYPE_GAMEFILE) && (!stricmp(GlobalTrackLocks[j].name,tempbuffer)) ){
				//ok, we found a possibly matching script...ask if they want that
				//checked in also...
				sprintf(buffer,"A possible level script (%s) was found, do you want\nto undo this check out also? (HIGHLY recommended)",tempbuffer);
				if(m_AutoCheckScripts || MessageBox(buffer,"Friendly Warning",MB_YESNO)==IDYES){
					//go ahead and delete the sucker!
					char t[256];
					sprintf(t,"UndoCheck Out: %s",tempbuffer);
...
bool IsScriptOutofDate(char *name)
{
	...
	if (hFindFilecpp != INVALID_HANDLE_VALUE){
		...
		if(hFindFiledll==INVALID_HANDLE_VALUE || CompareFileTime(&FindFileDatadll.ftLastWriteTime,&FindFileDatacpp.ftLastWriteTime)<0){
			//out of date
			out_of_date = true;
		}
		if (hFindFiledll != INVALID_HANDLE_VALUE) FindClose(hFindFiledll);
