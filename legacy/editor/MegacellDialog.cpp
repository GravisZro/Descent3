...
CMegacellDialog::CMegacellDialog(CWnd* pParent /*=NULL*/)
	: CDialog(CMegacellDialog::IDD, pParent)
{
		...
		else
		{
			...
			if (mng_FindSpecificMegacellPage (temp_pl.name,&megacellpage,0))
			{
				if (mng_AssignMegacellPageToMegacell(&megacellpage,n))
				{
					...
					OutrageMessageBox ("There was a problem loading this megacell.  You might encounter problems in dealing with it.  Good luck!");
...
void CMegacellDialog::OnCheckinMegacell() 
{
	...
	else 
	{
		...
		else
		{
			...
			int answer=MessageBox ("Do you wish to check in all the textures associated with this megacell (if any)?",Megacells[n].name,MB_YESNO);
...
void CMegacellDialog::OnDeleteMegacell() 
{
	...
	answer=MessageBox ("Do you wish to delete all the textures associated with this megacell?",Megacells[n].name,MB_YESNO);
...
void CMegacellDialog::OnRotateMegacell() 
{
	...
	for (i=0;i<MAX_MEGACELL_HEIGHT;i++)
	{
		...
		{
			buffer[((MAX_MEGACELL_HEIGHT-1)-t)*MAX_MEGACELL_WIDTH+i]=Megacells[n].texture_handles[i*MAX_MEGACELL_WIDTH+t];
		}
	}
