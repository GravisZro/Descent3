...
void FormatServerLine(char *fmt,int servernum,int pxonum)
{
	...
	sprintf(fmt,"%.20s\t\x02\x02b%s\x02\x45%.15s\x02\x63%d\x02\x6d%d/%d\x02\x7e%.3f",DLLNetwork_games[k].name,server_type,DLLNetwork_games[k].mission_name,
	...
	for(j=0;j<MAX_GAMELIST_ITEMS;j++)
	{
		if(PXOGamelist[j].used && (PXOGamelist[j].handle!=-1))
		{
			...
			if(gameid!=-1)
			{
				...
				char fmtline[200];
				FormatServerLine(fmtline,k,j);
				DLLSetUITextItemText(PXOGamelist[j].ti,fmtline,DLLNetwork_games[k].dedicated_server?UICOL_HOTSPOT_LO:GR_LIGHTGRAY);
			}
		}
	}
	...
	case MT_EVT_LOGIN:
		...
		*DLLMultiGameStarting = 0;
		...
		}
		{
			...
			if(UnvalidatedDLL)
			{
				DLLDoMessageBox(TXT_PXO_MASTERTRACKER,TXT_PXO_CANTAUTOVALIDATE,MSGBOX_OK,UICOL_WINDOW_TITLE,UICOL_TEXT_NORMAL);
				return;
			}
...
{	
	...
	void * main_wnd = DLLNewUIGameWindowCreate(TRACKER_MENU_X, TRACKER_MENU_Y, TRACKER_MENU_W, TRACKER_MENU_H, UIF_PROCESS_ALL | UIF_CENTER | NUWF_TITLEMED );
	...
	void * login_hs = DLLHotSpotCreate(main_wnd,UID_OK,KEY_ENTER,login_off_text,login_on_text,60,TRACKER_MENU_H-80, 60, 20,0); 
	
	void * cancel_hs = DLLHotSpotCreate(main_wnd,UID_CANCEL,KEY_ESC,cancel_off_text,cancel_on_text,130,TRACKER_MENU_H-80, 60, 20,0); 
	
	void * new_id_hs = DLLHotSpotCreate(main_wnd,4,KEY_N,new_prof_off_text,new_prof_on_text,0,45,120,20,UIF_CENTER|UIF_FIT); 
		...
		case UID_OK:
			{
				...
				if(valret == 1)
				{
					...
					for(int j=0;j<10;j++) PollPTrackNet();
...
int MainMultiplayerMenu ()
{
	...
	*DLLNewUIWindow_alpha = 255;
	...
	//void HotSpotCreate(int item,int parentitem, int id, int key, int txtitemoff, int txtitemon, int x, int y, int w, int h, int flags,int winnum)
...
#define JOIN_ROW	131									 
	void *join_lobby_hs = DLLHotSpotCreate(main_wnd,7 ,0  ,join_lobby_off_text ,join_lobby_on_text ,400,JOIN_ROW,170,15,0);
	void *join_chan_hs = DLLHotSpotCreate(main_wnd,14,0  ,join_chan_off_text ,join_chan_on_text ,178,JOIN_ROW,170,15,0);
	
	void *find_hs = DLLHotSpotCreate(main_wnd,9 ,0  ,find_pilot_off_text ,find_pilot_on_text ,156,403,140,15,0);
	void *get_pilot_hs = DLLHotSpotCreate(main_wnd,10,0  ,get_pilot_off_text,get_pilot_on_text,  colx1+10,372,130,15,0);
	...
	while (!exit_menu) 
	{
		...
		if(cmd)
		{
			...
			case CC_YOURCHANNEL:
				{
					...
					for(int l=0;l<18;l++) 
					{
						if(cleanlobby[l]=='_') cleanlobby[l] = ' ';
			...
			case CC_DISCONNECTED:
				...
				DLLDoMessageBox(TXT_PXO_MASTERTRACKER,TXT_PXO_CHATDISCONNECTED,MSGBOX_OK,UICOL_WINDOW_TITLE,UICOL_TEXT_NORMAL);		
				DLLNewUIWindowClose(main_wnd);
				exit_menu = 1;
				break;
			default:
		...
		if((DLLtimer_GetTime()-lastlisttime)>CHAT_USER_REFRESH_TIME)
		{
			...
			if(p)
			{
				...
				{
					if(user_ti[i]!=NULL) DLLRemoveUITextItem(user_ti[i]);	
					user_ti[i] = NULL;
				}
				char seps[] = " ";
				char *tokp;
		...
		if(p) 
		{
			...
			if(tokp)
			{
				...
				for(i=0;i<MAX_CHAT_CHANNELS;i++) 
				{
					if(chan_ti[i]!=NULL) DLLRemoveUITextItem(chan_ti[i]);	
					chan_ti[i] = NULL;
				}
				for(i=0;i<MAX_CHAT_CHANNELS;i++) 
				{
					...
					//if(pcount)
					{
						...
						*pcount = NULL;
						...
						for(int l=0;l<18;l++) 
						{
							if(chan_info[i].name[l]=='_') chan_info[i].name[l] = ' ';
							if(chan_info[i].name[l]=='\0') chan_info[i].name[l]= ' ';
						}
						pcount++;					
						char *ptopic = pcount;
...
int SearchMasterTrackerGameMenu ()
{
	...
	*DLLNewUIWindow_alpha = 255;
	...
	void * screen_header = DLLTextCreate(main_wnd,game_hdr_text,5,15,UIF_CENTER); cury+=30;
	void * start_hs = DLLHotSpotCreate(main_wnd,7,KEY_S,start_off_text,start_on_text,320,cury,150,15,UIF_CENTER);cury+=30;//25;

	int setdfty;	

	return_hs = DLLHotSpotCreate(main_wnd,8,KEY_R,return_text_off,return_text_on,490,cury,250,15,UIF_CENTER);cury+=20;	
	...
	if(Bypass_chat)
	{
		...
		setdfty = cury;
		set_dft_hs = DLLHotSpotCreate(main_wnd,9,0,default_off_text,default_on_text,490,cury,300,15,UIF_CENTER);cury+=20;
	}
	else
	{
		sprintf(fmt_textstr,TXT_PXO_MAKEGAMEDEFAULT,28);
		...
		default_off_text = DLLCreateNewUITextItem(fmt_textstr,UICOL_HOTSPOT_LO);
		setdfty = cury;
		set_dft_hs = DLLHotSpotCreate(main_wnd,9,0,default_off_text,default_on_text,490,cury,300,15,UIF_CENTER); cury+=30;
	}
	//void * game_head = DLLTextCreate(main_wnd,game_head_text,45,cury,0); 

	void * gname_hs = DLLHotSpotCreate(main_wnd,SORT_GAMENAME,0,gname_off_text,gname_on_text,44,cury,DLLgrtext_GetTextLineWidth(TXT_PXO_GAMENAME2)+1,15,0);
	void * gtype_hs = DLLHotSpotCreate(main_wnd,SORT_GAMETYPE,0,gtype_off_text,gtype_on_text,215,cury,DLLgrtext_GetTextLineWidth(TXT_PXO_GAMETYPE)+1,15,0);
	void * msn_hs = DLLHotSpotCreate(main_wnd,SORT_MISSION,0,msn_off_text,msn_on_text,320,cury,DLLgrtext_GetTextLineWidth(TXT_PXO_MISSION)+1,15,0);
	void * lvl_hs = DLLHotSpotCreate(main_wnd,SORT_LEVEL,0,lvl_off_text,lvl_on_text,428,cury,DLLgrtext_GetTextLineWidth(TXT_PXO_LEVEL)+1,15,0);
	void * players_hs = DLLHotSpotCreate(main_wnd,SORT_PLAYERS,0,plrs_off_text,plrs_on_text,479,cury,DLLgrtext_GetTextLineWidth(TXT_PXO_PLAYERS)+1,15,0);
	void * ping_hs = DLLHotSpotCreate(main_wnd,SORT_PING,0,ping_off_text,ping_on_text,549,cury,DLLgrtext_GetTextLineWidth(TXT_PXO_PING)+1,15,0);	
	
	cury+=15;
	void * game_list = DLLListCreate(main_wnd,6,10,cury,600,230,UIF_CENTER|UILB_NOSORT);cury+=260;
		...
		case 9:
			//	Change the current selection
			{
				...
				if(bypass_chat_curr_val)
				{
					...
					sprintf(fmt_textstr,TXT_PXO_MAKEGAMEDEFAULT,28);
					default_off_text = DLLCreateNewUITextItem(fmt_textstr,UICOL_HOTSPOT_LO);
					set_dft_hs = DLLHotSpotCreate(main_wnd,9,0,default_off_text,default_on_text,490,setdfty,300,15,UIF_CENTER);
				}
				else
				{
	...
	if(DLLNetgame->flags & NF_TRACK_RANK)
	{
		...
		if(MTWritingPilot!=-1)
		{
			...
			if(rcode!=0)
			{
				if(rcode==1)
				{
					...
					DLLmprintf ((0,"Pilot data wrote to the Mastertracker for Player[%d] %s (tid=%s).\n",MTWritingPilot,DLLMTPilotinfo[MTWritingPilot].pilot_name,DLLMTPilotinfo[MTWritingPilot].tracker_id));
					MTWritingPilot = -1;
		...
		else
		{
			...
			for (i=0;i<MAX_NET_PLAYERS;i++)
			{
				...
				{
					if((!(DLLMNetPlayers[i].flags  & NPF_MT_READING_PILOT))&&(DLLMNetPlayers[i].flags  & NPF_MT_HAS_PILOT_DATA))
					{
			...
			if(MTReadingPilot!=-1)
			{
				...
				if(rcode!=0)
				{
					if(rcode==1)
					{
						...
						DLLmprintf ((0,"Read pilot %s rank of %f\n",DLLMPlayers[MTReadingPilot].callsign,DLLMPlayers[MTReadingPilot].rank));
						...
						DLLmprintf ((0,"Pilot data received from the Mastertracker for Player[%d] %s (tid=%s).\n",MTReadingPilot,DLLMTPilotinfo[MTReadingPilot].pilot_name,DLLMTPilotinfo[MTReadingPilot].tracker_id));
						MTReadingPilot = -1;
					}
					else
					{
...
int JoinNewLobby(char *lobby)
{
	...
	void * title = DLLTextCreate(main_wnd,title_text,0,7,UIF_CENTER); 
	void * cancel_hs = DLLHotSpotCreate(main_wnd,5 ,KEY_ESC,cancel_off_text ,cancel_on_text , 20,100,70,15,UIF_CENTER);
	DLLNewUIGameWindowOpen(main_wnd);
	do
	{
...
char *SendWhisper(char *name)
{
	...
	//HotSpotCreate(int item,int parentitem, int id, int key, int txtitemoff, int txtitemon, int x, int y, int w, int h, int flags,int winnum)
	void * cancel_hs = DLLHotSpotCreate(main_wnd,UID_CANCEL,KEY_ESC,cancel_on_text,cancel_off_text,130,256-80, 60, 20,0); 
	...
	if(tokp)
	{
		*tokp = NULL;
		...
		if(iresult==1)
		{
			...
			//Do rank thingy here.
			sprintf(tmp_text,"Rank: %s (%d) Next Advance at %d",GetRankString(d3_pilot_info.rank/65536),d3_pilot_info.rank/65536,GetNextRank(d3_pilot_info.rank/65536));
			...
			if(d3_pilot_info.kills && d3_pilot_info.deaths)
			{
				...
				leff = (int)((float)((float)d3_pilot_info.kills/((float)d3_pilot_info.deaths+(float)d3_pilot_info.kills+(float)d3_pilot_info.suicides))*100.0);
			}			
			...
			cury = 30;
			pilot_t = DLLTextCreate(main_wnd,pilot_name_text,0,cury,UIF_CENTER); cury+=30;
				
			rank_t = DLLTextCreate(main_wnd,rank_text,0,cury,UIF_CENTER); cury+=20;
			kill_t = DLLTextCreate(main_wnd,kills_text,0,cury,UIF_CENTER); cury+=20;
			death_t = DLLTextCreate(main_wnd,deaths_text,0,cury,UIF_CENTER); cury+=20;
			suicide_t = DLLTextCreate(main_wnd,suicide_text,0,cury,UIF_CENTER); cury+=20;
			eff_t = DLLTextCreate(main_wnd,eff_text,0,cury,UIF_CENTER); cury+=20;
			time_in_game_t = DLLTextCreate(main_wnd,time_text,0,cury,UIF_CENTER); cury+=30;
...
int ShowMessageOfTheDay(void)
{
	...
	close_hs = DLLHotSpotCreate(main_wnd,UID_OK,KEY_ENTER,close_off_text,close_on_text,60,MOTD_HEIGHT-60, 60, 20,UIF_CENTER); 
